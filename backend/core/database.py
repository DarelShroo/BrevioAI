...
class DB:
    def __init__(self, uri: str = None):
        try:
            ...
            server_selection_timeout = self._get_env_int("SERVER_SELECTION_TIMEOUT_MS", 30000)
            connect_timeout = self._get_env_int("CONNECT_TIMEOUT_MS", 20000)
            socket_timeout = self._get_env_int("SOCKET_TIMEOUT_MS", 60000)

            logger.info(f"Initializing MongoDB client with URI: {connection_uri[:20]}...")
    ...
    def _get_env_int(self, var_name: str, default: int) -> int:
        try:
            ...
            logger.warning(f"Invalid integer for {var_name}: {str(e)}. Using default: {default}")
            return default
    ...
    def _verify_connection(self) -> None:
        try:
            ...
            logger.error(f"Unexpected error verifying connection: {str(e)}", exc_info=True)
            raise RuntimeError(f"Unexpected error during connection verification: {str(e)}") from e
    ...
    def database(self, database: str = 'brevio') -> Database:
        try:
            ...
            raise PermissionError(f"No permission to access database {database}: {str(e)}") from None
        except ValueError as e:
            logger.error(f"Invalid database name: {str(e)}")
            raise ValueError(f"Invalid database name: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error accessing database {database}: {str(e)}", exc_info=True)
