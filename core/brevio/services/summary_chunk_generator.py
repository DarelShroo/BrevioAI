...
class SummaryChunkGenerator:
    ...
    async def generate_chunk(
        self, request: ChunkProcessingRequest, client: AsyncOpenAI
        ...
        try:
            ...
            previous_context_prompt = await self._get_previous_context_prompt(
                request.accumulated_summary, request.language, encoder
            )
            ...
            try:
                ...
                output_tokens = self._record_history(
                    full_prompt, request.chunk, response, encoder, TypeCall.SUMMARY
                )

            except asyncio.TimeoutError:
                logger.warning(
                    f"Timeout processing chunk {request.index}, requeueing..."
                )
            ...
            logger.warning(
                f"Retryable error on chunk {request.index}: {e}, requeueing..."
            )
            await self._requeue_chunk(request)
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error on chunk {request.index}: {e}", exc_info=True
            )
            return request.index, None, 0

    async def _get_previous_context_prompt(
        self, accumulated_summary: str, language: LanguageType, encoder: Any
    ) -> str:
