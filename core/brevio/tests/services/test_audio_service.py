...
@pytest.fixture
def caplog(caplog: Any) -> Any:
    """Fixture that should set the logging level for audio_service tests."""
    caplog.set_level(logging.DEBUG, logger="core.brevio.services.audio_service")
    return caplog
...
@pytest.mark.asyncio
async def test_get_media_info_success(audio_service: AudioService, caplog: Any) -> None:
    ...
    with patch(
        "core.brevio.services.audio_service.subprocess.run", return_value=mock_result
    ):
        result = await audio_service.get_media_info("/path/to/test.mp3")
        assert result == {
            "title": "test",
            "duration": pytest.approx(62.0575),
        }
...
@pytest.mark.asyncio
async def test_get_media_info_no_duration(
    audio_service: AudioService, caplog: Any
) -> None:
...
@pytest.mark.asyncio
async def test_get_media_info_yt_success(
    audio_service: AudioService, caplog: Any
) -> None:
    ...
    with patch(
        "core.brevio.services.audio_service.yt_dlp.YoutubeDL", return_value=mock_ydl
    ):
        ...
        assert (
            "Fetching YouTube media info from URL: https://youtube.com/watch?v=test"
            in caplog.text
        )
        assert (
            "Extracted YouTube info: {'title': 'Test Video', 'duration': 3661}"
            in caplog.text
        )


@pytest.mark.asyncio
async def test_get_media_info_yt_no_info(
    audio_service: AudioService, caplog: Any
) -> None:
    ...
    with patch(
        ...
        assert (
            "Fetching YouTube media info from URL: https://youtube.com/watch?v=invalid"
            in caplog.text
        )
        assert (
            "No info extracted from YouTube URL: https://youtube.com/watch?v=invalid"
            in caplog.text
        )


@pytest.mark.asyncio
async def test_get_media_info_exception(audio_service: AudioService) -> None:
    """Should raise an exception if an error occurs in get_media_info."""
    with patch(
        "core.brevio.services.audio_service.subprocess.run",
        ...
        assert (
            "Ha ocurrido un error inesperado al obtener información del video"
...
@pytest.mark.asyncio
async def test_get_media_info_yt_exception(
    audio_service: AudioService, caplog: Any
) -> None:
    """Should raise an exception if an error occurs in get_media_info_yt."""
    with patch(
        "core.brevio.services.audio_service.yt_dlp.YoutubeDL",
        ...
        assert (
            "Ha ocurrido un error inesperado al obtener información del video https://youtube.com/watch?v=test"
            in str(exc_info.value)
        )
        ...
        assert (
            "Error fetching YouTube info from https://youtube.com/watch?v=test: YouTube API error"
            in caplog.text
        )
