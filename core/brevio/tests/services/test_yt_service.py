...
@pytest.fixture
def yt_service() -> YTService:
    return YTService()


@pytest.mark.asyncio
async def test_download_success(yt_service: YTService) -> None:
    ...
    with patch("yt_dlp.YoutubeDL") as mock_yt:
        ...
        result = await yt_service.download(
            HttpUrl("https://www.youtube.com/watch?v=dQw4w9WgXcQ"), "./backend/audios"
        )
...
async def test_process_video(yt_service: YTService) -> None:
    ...
    with patch.object(yt_service, "download", new_callable=AsyncMock) as mock_download:
        ...
        mock_download.assert_called_once_with(video_url, "../..data", None)


@pytest.mark.asyncio
async def test_process_videos(yt_service: YTService) -> None:
...
@pytest.mark.asyncio
async def test_get_video_urls_from_playlist(yt_service: YTService) -> None:
...
@pytest.mark.asyncio
async def test_get_media_duration_playlist(yt_service: YTService) -> None:
...
@pytest.mark.asyncio
async def test_count_media_in_yt_playlist(yt_service: YTService) -> None:
...
@pytest.mark.asyncio
async def test_get_video_duration(yt_service: YTService) -> None:
...
@pytest.mark.asyncio
async def test_get_media_duration_video(yt_service: YTService) -> None:
