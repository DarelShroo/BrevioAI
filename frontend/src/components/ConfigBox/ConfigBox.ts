...
import type { LanguageTypeReponse } from '@/interfaces/language-response'
import type { ModelTypeReponse } from '@/interfaces/model-response'
import { useStoreNotification } from '@/composables/useStoreNotification'
export default defineComponent({
  async setup() {
    const languageOptions = ref<LanguageTypeReponse[]>([]);
    const modelOptions = ref<ModelTypeReponse[]>([]);
    
    const fetchLanguagesList = async (): Promise<LanguageTypeReponse[]> => {
      try {
    ...
    const fetchModelsList = async (): Promise<ModelTypeReponse[]> => {
      try {
